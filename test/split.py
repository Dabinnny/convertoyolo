...
out_txt_path = "D:/2.dataset/coco/map/result/"
...
def make_list():
    with open (val_txt, 'r') as f:
        lines = f.readlines()
    lines = list(map(lambda x: os.path.basename(x).rstrip().replace("jpg", "txt"), lines))
    data = list(map(lambda x: x.rstrip(), lines))

    for i in data:
        copy_file = os.path.join(txt_path, i)
        copyfile(copy_file, os.path.join(out_txt_path, i))
