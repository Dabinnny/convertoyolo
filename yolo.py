...
def convert_yolo_to_ls(input_dir, out_file,
                       ...
                       image_root_url='/data/upload/6/', image_ext='.jpg'):
    ...
    df = pd.read_csv('D:/test/task.csv')[100:200]

    split = df.data.str.split('/').apply(lambda x: pd.Series(x))
    split = split[4].str.split('.').apply(lambda x: pd.Series(x))
    split = split[0].str.split('-').apply(lambda x: pd.Series(x))
    
    path_dic = dict(zip(split[1], split[0]))
    
    for f in os.listdir(labels_dir):
        for key, value in path_dic.items():    
            if key == f[0:-4]:
                image_file_base = value + '-' + f[0:-4] + image_ext
                image_file = os.path.join(input_dir, 'images', f[0:-4] + image_ext)
